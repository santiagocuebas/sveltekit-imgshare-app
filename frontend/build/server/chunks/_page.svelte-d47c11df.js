import { c as create_ssr_component, v as validate_component, e as escape, d as add_attribute, h as each } from './index2-1f46a731.js';
	...
	<div class="settings-options svelte-3w2akr">${validate_component(OptionSettings, "Option").$$render($$result, { title: "Set Avatar" }, {}, {
      ...
      default: () => {
        return `<form id="description" action="${escape(DIR, true) + "/api/settings/description"}" method="POST" class="svelte-3w2akr"><label for="description">Change the description:</label>
				<textarea id="description" name="description" rows="5" spellcheck="false" autocomplete="off" class="svelte-3w2akr">${data.user.description || ""}</textarea>
      ...
      default: () => {
        return `<form id="password" action="${escape(DIR, true) + "/api/settings/password"}" method="POST" class="svelte-3w2akr"><label for="actPassword">Enter the actual password:</label>
				<input type="password" name="actPassword" placeholder="Actual Password" class="svelte-3w2akr">
				<label for="newPassword">Enter the new password:</label>
				<input type="password" name="password" placeholder="New Password" class="svelte-3w2akr">
				<label for="confirmPassword">Confirm the new password:</label>
				<input type="password" name="confirmPassword" placeholder="Confirm Password" class="svelte-3w2akr">
				<button class="svelte-3w2akr">Send</button></form>`;
      }
    })}
		<span class="svelte-3w2akr"></span>
      ...
      default: () => {
        return `<form id="link" action="${escape(DIR, true) + "/api/settings/link"}" method="POST" class="svelte-3w2akr"><label for="title">Enter the title:</label>
				<input type="text" name="title" placeholder="Title" class="svelte-3w2akr"${add_attribute("value", title, 0)}>
				<label for="url">Enter the URL:</label>
				<input type="url" name="url" placeholder="URL" maxlength="4000" class="svelte-3w2akr"${add_attribute("value", url, 0)}>
				<button class="svelte-3w2akr">Send</button></form>
...
//# sourceMappingURL=_page.svelte-d47c11df.js.map
