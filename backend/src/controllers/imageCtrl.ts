...
export const postPublic: Direction = async (req, res) => {
	...
	if (
		image !== null &&
		(username === image.author || role !== UserRole.EDITOR)
	) {
...
export const postLike: Direction = async (req, res) => {
	...
	if (image !== null) {
		if (req.body.like === 'like') {
			const [ actLike, actDislike ] = getLike(image.like, image.dislike, username);
			image.like = actLike;
			image.dislike = actDislike;
		} else if (req.body.like === 'dislike') {
			const [ actDislike, actLike ] = getLike(image.dislike, image.like, username);
			image.like = actLike;
			image.dislike = actDislike;
		}
...
export const postComment: Direction = async (req, res) => {
	...
	if (
		image !== null && typeof comment === 'string' &&
